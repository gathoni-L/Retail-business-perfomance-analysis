...
select c1.first_name as customer_1,c2.first_name as customer_2,c1.membership_status
...
select s.customer_id,p.product_name,sum(s.total_amount) as total_sales
from assignment.sales s 
right join assignment.products p  on s.product_id =p.product_id 
group by p.product_name,s.customer_id  ;
...
--(Use of IN as it matches values in a list)
...
select MAX(s.Total_amount) as expensive_product,p.product_name 
from assignment.sales s 
join assignment.products p on s.product_id =p.product_id
group by p.product_name ;
...
select c.first_name,c.membership_status ,s.total_amount,s.sale_date,p.product_name  
from assignment.customers c 
join assignment.sales s on c.customer_id =s.customer_id 
join assignment.products p on s.product_id =p.product_id
...
from assignment.products p 
join assignment.inventory i on i.product_id = p.product_id 
...
select AVG(s.quantity_sold),p.product_name,p.product_id
from assignment.sales s 
join assignment.products p on p.product_id =s.product_id 
...
select p.product_name,i.stock_quantity, s.sale_id 
from assignment.products p 
join assignment.inventory i on i.product_id = p.product_id 
...
select p.product_name,s.total_amount,s.sale_id
from assignment.customers c 
join assignment.sales s on c.customer_id =s.customer_id 
join assignment.products p on s.product_id =p.product_id
...
select sale_id,max(quantity_sold)
from assignment.sales s
group by sale_id
having s.quantity_sold >1;

-- 38. Write a query to find the total quantity of products sold per customer.
select sale_id,sum(quantity_sold)
from assignment.sales s
group by sale_id;
...
select product_name,max(stock_quantity) as maximum_stock,
min(stock_quantity) as minimum_quantity
from assignment.products p
group by product_name,p.stock_quantity 
...
select c.customer_id,p.product_name,s.total_amount,c.membership_status 
from assignment.products p 
...
select p.product_name,s.sale_date,sum(s.total_amount)as total_sales
from assignment.sales s 
join assignment.products p on s.product_id = p.product_id
...
join assignment.sales s on i.product_id =s.product_id 
...
select p.product_name, s.total_amount,s.quantity_sold
from assignment.products p 
join assignment.sales s on p.product_id = s.product_id
where s.total_amount between 200 and 800;

-- 49. Write a query to find the customers who spent the most money in the year 2023.
select s.sale_id,c.first_name,s.sale_date _date,max(s.total_amount)
from assignment.customers c 
join assignment.sales s on c.customer_id =s.customer_id 
group by s.sale_id,c.first_name,s.sale_date
...
from assignment.sales s 
where s.product_id >100
	and s.total_amount >200;